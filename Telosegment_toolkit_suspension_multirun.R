############################################ Telosegment toolkit for suspension cells ############################################################
...
# This version uses the additional function evaluateZProfile() to identify cells which are not completely in the focus volume
# These cells are annotated and can later be filtered out during plotting
# The DAPI intensity profiles along z and the according annotation are exported as additional CSV files in the subfolder "nuc_focuscheck"
# The column "infocus" containing "y"(yes) or "n"(no) annotation will be added to the exported _nucleus_spotmask_npmask_area_intensity_results.csv table
...
scriptversion <- list.files(getwd(),pattern = "Telosegment_toolkit_suspension_multirun_4ch_v1.R")
...
dir.create(paste0(folder,date_time,"_Telosegment_toolkit_suspension_results"))
results_folder <- paste0(folder,date_time,"_Telosegment_toolkit_suspension_results")
...
#create folder for exporting nucleus focus-check data
dir.create(paste0(results_folder,"/",date_time,"_nuc_focuscheck"))
...
# nucleus focus check annotation vector
infocus <- vector()
...
for(i in 1:length(positions)){
  ...
  ######################################  Compute and export data for nucleus focus check ################################################
  nucfocus <- evaluateZProfile(nuc_data,nucmask,zperch)
  write.table(file=paste0(results_folder,"/",date_time, "_nuc_focuscheck","/",gsub(".tif","_nucZprofile_annotated.csv",positions[i])), nucfocus, sep = "\t", append = F, row.names = F, quote = F)
  # collect annotation data in the infocus vector
  infocus <- c(infocus, as.character(nucfocus[(zperch+1),]))
...
int_area_res <- cbind(pos[1:(cnt-1)],nucleus_features$cell,nucleus_features$s.area, area_all_spotmask[1:(cnt-1)],area_npmask[1:(cnt-1)], num_spots[1:(cnt-1)],
                      ...
                      mean_nuclear_ch2_intensity[1:(cnt-1)], median_nuclear_ch2_intensity[1:(cnt-1)], sd_nuclear_ch2_intensity[1:(cnt-1)],mean_ch2_all_spotmask_intensity[1:(cnt-1)],sd_ch2_all_spotmask_intensity[1:(cnt-1)],mean_ch2_npmask_intensity[1:(cnt-1)],sd_ch2_npmask_intensity[1:(cnt-1)],infocus)
...
colnames(int_area_res) <- c("position","cell", "area_nucmask", "area_all_spotmask", "area_npmask","num_spots",
                            ...
                            "mean_nuclear_ch2_intensity", "median_nuclear_ch2_intensity", "sd_nuclear_ch2_intensity","mean_ch2_all_spotmask_intensity","sd_ch2_all_spotmask_intensity","mean_ch2_npmask_intensity","sd_ch2_npmask_intensity","infocus")
